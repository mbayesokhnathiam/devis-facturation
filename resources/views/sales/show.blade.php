@extends('layouts.app', ['page' => 'Gestion des ventes', 'pageSlug' => 'sales', 'section' => 'transactions'])
...
@section('content')
                    ...
                    <div class="row">
                        ...
                        @if (!$sale->finalized_at)
                            <div class="col-4 text-right">
                                ...
                                @else
                                    <button type="button" class="btn btn-sm btn-primary" onclick="confirm('ATTENTION: Souhaitez-vous finaliser cette vente? Vos enregistrements ne peuvent plus être modifiés à partir de maintenant.') ? window.location.replace('{{ route('sales.finalize', $sale) }}') : ''">
                                        Finaliser vente
                                    </button>
                                @endif
                            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        ...
                        @if (!$sale->finalized_at)
                            <div class="col-4 text-right">
                                <a href="{{ route('sales.product.add', ['sale' => $sale->id]) }}" class="btn btn-sm btn-primary">Ajouter produit</a>
                            </div>
                        @endif
                    </div>
                </div>
                            ...
                            @foreach ($sale->products as $sold_product)
                                <tr>
                                    ...
                                    <td class="td-actions text-right">
                                        @if(!$sale->finalized_at)
                                            <a href="{{ route('sales.product.edit', ['sale' => $sale, 'soldproduct' => $sold_product]) }}" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="Modifier la commande">
    ...
    <div class="row">
        <div class="col-md-12">
            <a href="{{ route('sale.print.options', ['sale' => $sale]) }}" class="btn btn-sm btn-primary">
                Imprimer facture
            </a>
            @if(!$sale->validate)
                <a href="{{ route('sales.product.return.data', ['sale' => $sale]) }}" class="btn btn-sm btn-primary">
                    Modifier facture
                </a>
            @endif
            @if(!$sale->paid)
            <form action="{{ route('sales.cachein', ['sale' => $sale, 'soldproduct' => $sold_product]) }}" method="post" class="d-inline">
                @csrf
                @method('post')
                <button type="button" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="Encaisser paiement client" onclick="this.parentElement.submit()">
                    Encaisser
                </button>
            </form>
