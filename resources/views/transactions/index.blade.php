                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($transaction->client)
                                                <a href="{{ route('clients.show', $transaction->client) }}">{{ $transaction->client->name }}<br>TEL-{{ $transaction->client->phone }}</a>
                                            @else
                                                Ne s'applique pas
                                            @endif
                                        </td>
                                        ...
                                        <td class="td-actions text-right">
                                            ...
                                            @elseif ($transaction->transfer_id)
                                                <a href="{{ route('transfer.show', $transaction->transfer) }}" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="Plus de détails">
                                                    <i class="tim-icons icon-zoom-split"></i>
                                                </a>
                                            @else
                                                ...
                                                {{-- <form action="{{ route('transactions.destroy', $transaction) }}" method="post" class="d-inline">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="button" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="Supprimer transaction" onclick="confirm('Voulez-vous vraiment supprimer cette transaction?') ? this.parentElement.submit() : ''">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('transactions.create', ['type' => 'payment']) }}" class="btn btn-sm btn-primary">Paiement</a>
                        <a href="{{ route('transactions.create', ['type' => 'income']) }}" class="btn btn-sm btn-primary">Réception</a>
                        <a href="{{ route('transactions.create', ['type' => 'expense']) }}" class="btn btn-sm btn-primary">Dépense</a>
                        <a href="{{ route('sales.create') }}" class="btn btn-sm btn-primary">Vente</a>
